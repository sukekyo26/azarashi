...
- `common/` — 全環境共通の配布ペイロード
  ...
  - `.claude/settings.fragment.json`, `.claude.fragment.json`（MCP: serena）, `.claude/hooks/`（Claude Code 専用の hook）, `.claude/statusline.sh`（表示の説明は `docs/statusline.md`）, `.claude/output-styles/`
