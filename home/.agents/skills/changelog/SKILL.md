...
二言語の CHANGELOG を持つプロジェクトでは、両ファイルを常に同期させること（同じ内容を各言語で記載する）。
...
- **カテゴリ見出しは以下の 4 種類のみ使用する。この順序で並べる。他のカテゴリ（`Security` / `Deprecated` / `Docs` 等）を勝手に追加しない**
  - 英語: `Added` → `Changed` → `Fixed` → `Removed`
  - 日本語: `追加` → `変更` → `修正` → `削除`
...
- テスト追加・テストカバレッジ閾値の変更
- CI/CD 設定の変更（ワークフロー修正、action の SHA pin 更新等）
- テスト用内部 API の追加（DI ヘルパ等）
...
2. CHANGELOG の `## [Unreleased]` セクション全体を読み、既存のカテゴリ見出し（`### Added` 等）を把握する
...
6. 二言語の CHANGELOG を持つ場合、もう一方のファイルの `## [Unreleased]` セクションも同様に整理・追記する
