...
Claude.ai Pro / Max の契約者（または支出上限付き gateway 経由）だけに stdin の `rate_limits` が来る。Bedrock や API キーでは項目自体が無いので何も出ない。ゲージはコンテキスト使用率と同じ色分け（緑 < 60% ≤ 黄 < 80% ≤ 赤）。2 行目のゲージと上下で繋がって見えないよう、下寄せの半分の高さの文字で描く。括弧はその枠がリセットされる時刻で、5 時間枠は時刻、7 日枠は日付と時刻。支出上限（`spend`）があればその後ろに付く。リセット時刻を過ぎた枠は Claude Code 側が落とすので消える。
