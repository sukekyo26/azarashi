...
| `💥miss 40k $0.25` | 直近ターンでプレフィックスが壊れ、キャッシュを書き直した（後述） |
...
- `40k`: 当ターンの cache_creation トークン数（書き直した量）
- `$0.25`: その書き直しの料金。`cache_creation × 入力単価 × write 係数（5m: 1.25 / 1h: 2）× 地域係数`。価格表は Bedrock コスト再計算と同じもの

セッション最初のターンは比較対象が無いので miss にならない。`/compact` 直後や TTL 切れ後の再構築も miss として出る（その分の料金が実際に発生している）。

miss が出たとき考えられる原因は、権限モードの切替、CLAUDE.md や memory の編集、MCP サーバーやツール定義の変化、API 側のリトライ。usage には system prompt の内訳が無いので、statusline から原因までは特定できない。
