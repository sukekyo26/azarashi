...
front / mid には原因候補として、直前ターンから権限モード（`permission-mode`）や `mode` の**値が変わったか**と、ミスがユーザー入力の境界（`user-prompt`）かツールループ中（`tool-loop`）かを付ける。`permission-mode` / `mode` / `bridge-session` のレコード自体は入力のたびに書かれる帳簿なので、存在だけでは何の根拠にもならない。usage には system prompt の内訳が無いので、候補以上のことは言えない。
...
- warm 中のミス 20 件のうち 17 件はユーザー入力の境界で起きていて、ツールループ中は 3 件。権限モードの値が実際に変わっていたのは 1 件だけで、切替が原因とは言えない。入力時に Claude Code が system prompt 側で何を変えるかは transcript からは分からない。
- mid の `read` は 26448 で複数セッション共通。静的な system 部分の直後（CLAUDE.md・memory・添付の注入領域）が変わっている。
- ティアは全セッション合計で 1h の方が安い（5m 換算 789 USD に対し 1h 換算 745 USD）。セッション単位では 5m が安いものが多数だが、長いセッションの放置コストが支配的。現状の 1h 設定は維持でよい。

## 運用上の結論

- このリポジトリの hook はプレフィックスを壊していない。additionalContext を出す hook が無く、UserPromptSubmit / PreToolUse の出力は会話末尾に付くだけ。
- warm 中のミスは入力境界に集中するが、原因は transcript から特定できない。件数もコストも小さいので、ルールを増やすより点検で足りる。
- ティア（`ENABLE_PROMPT_CACHING_1H`）は 1h 維持。
