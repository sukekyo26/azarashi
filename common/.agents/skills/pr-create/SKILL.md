...
- **テンプレート遵守** — 採用したテンプレートのセクション構成・順序を維持し、空欄を残さない（該当が無ければ `なし` / `None` と明示）。
...
PR の種別ごとにテンプレートを次の順で探し、最初に見つかった 1 つを読み込んで各セクションを埋める。リポジトリ側を常に優先する（プロジェクト固有の項目があるため）。

| 種別 | 探索順 |
|:-----|:------|
| 通常の開発 PR（→ `develop`） | `.github/pull_request_template.md` → `~/.github/pull_request_template.md` → 無ければ「概要 / 変更点 / 動作確認」の簡潔な本文を生成 |
| リリース PR（`develop` → `main`） | `.github/PULL_REQUEST_TEMPLATE/release.md` → `~/.github/PULL_REQUEST_TEMPLATE/release.md` → 無ければ通常テンプレートにフォールバック |

**観点はテンプレートに従って切り替える。** 通常 PR は「この差分が何を変えるか」（Summary / 変更の種別 / 変更点）を書き、リリース PR は「このリリースで何が世に出るか」（バージョンの上げ幅と根拠 / 当該リリースの CHANGELOG セクションを転記した Released changes / 破壊的変更と移行手順 / リリース前チェックリスト）を書く。リリース PR に個々のコミットの実装詳細を並べない。
...
- [ ] 種別に合ったテンプレート（通常 / リリース）を選び、全セクションを埋め、空欄なし
- [ ] (リリース PR の場合) `Released changes` が CHANGELOG の当該バージョン節と一致し、リリース前チェックリストを確認した
