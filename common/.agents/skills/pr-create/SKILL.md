...
- **テンプレート遵守** — `.github/pull_request_template.md` があればそのセクション構成・順序を維持する。空欄を残さない (`関連 issue` / `破壊的変更の詳細` がなければ "なし" と明示)。テンプレートが無ければ「概要 / 変更点 / 動作確認」の簡潔な本文を生成する。**リリース PR（`develop → main` かつバージョンアップを含む）では、リポジトリにリリース専用テンプレート（`.github/PULL_REQUEST_TEMPLATE/release.md` 等）があればそれを優先する。**
- **タイトルは Conventional Commits** — `feat(scope): ...` / `fix(scope): ...` / `chore: ...` 等。`develop → main` の PR でも、バージョンアップを含む場合は **`develop` 上のリリースコミットと同一タイトル**を採用する（プロジェクト規約により `feat: release vX.Y.Z` / `chore: release vX.Y.Z` 等。ハードコードせずコミットから取る）。含まない場合（リファクタ・chore のみ）はコミット内容を集約した通常タイトルにする。
...
| `develop → main` かつバージョンアップあり | `develop` のリリースコミットと同一タイトル（例: `feat: release vX.Y.Z`。プロジェクト規約に従い、ハードコードしない） |
| `develop → main` かつバージョンアップなし | コミット内容を集約した通常タイトル (例: `refactor: consolidate internal modules`) |

### 6. PR 本文の生成

`.github/pull_request_template.md` があれば読み込み、各セクションを埋めて生成する。**リリース PR でリリース専用テンプレート（`.github/PULL_REQUEST_TEMPLATE/release.md` 等）がある場合はそちらを読み込み、そのテンプレート自身の節構成に従って埋める**（例: リリーステンプレなら「Released changes」に当該リリースの CHANGELOG セクションを転記し、Release checklist を確認する）。この場合、以下 6.1（変更の種別チェックボックス）はテンプレートに当該節があるときのみ適用する。
...
- **概要**: この差分が何をするかを 1〜2 文。変更後の状態を書き、そこに至った経緯・背景の物語は書かない。
- **変更点**: `git diff origin/<base>...HEAD --stat` で実際に変わったファイルを確認し、その差分だけを外部から見える単位で整理する。コミットログ (`git log origin/<base>..HEAD`) は補助に留める — 途中で入れて撤回した変更はログに残っていても差分には無い。
...
- [ ] (テンプレートに `変更の種別` 節がある場合) チェックボックスは **全項目を残し**、該当する物だけ `[x]` にしている (チェックされない項目を削除していない)
- [ ] (テンプレートに `変更の種別` 節がある場合) 最低 1 つ `[x]` が付いている
