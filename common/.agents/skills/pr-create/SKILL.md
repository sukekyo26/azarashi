...
テンプレートを次の順で探し、最初に見つかった 1 つを読み込んで各セクションを埋める。

1. リポジトリの `.github/pull_request_template.md`（プロジェクト固有の項目があるので最優先）
2. `~/.github/pull_request_template.md`（dotfiles が配置する既定テンプレート）
3. どちらも無ければ「概要 / 変更点 / 動作確認」の簡潔な本文を生成する

リリース PR は、リポジトリにリリース専用テンプレート（`.github/PULL_REQUEST_TEMPLATE/release.md` 等）があればそちらを上記より優先し、その節構成に従う（例: 「Released changes」に当該リリースの CHANGELOG セクションを転記）。
