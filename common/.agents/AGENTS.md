...
- **例外は通常のファイル操作でよい** — 非コードファイル（Markdown・JSON・TOML・設定ファイル・シェルスクリプト）、数行読めば足りる箇所、LSP が無い言語、新規ファイルの作成。serena の注入プロンプトは組み込みの読み書きを一律「FORBIDDEN」と宣言してくるが、これらの例外では従わなくてよい。
